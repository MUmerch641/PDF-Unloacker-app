...
import { StyleSheet, Text, View, TouchableOpacity, ActivityIndicator, Alert, Platform, Modal } from 'react-native';
...
const LAN_IP = '10.154.236.86';

const API_URL = Platform.OS === 'web'
  ? 'http://localhost:8000/unlock-pdf'
  : `http://${LAN_IP}:8000/unlock-pdf`;

// PDF.js Viewer HTML for Android (since WebView can't render PDF natively on Android)
...
<head>
    ...
    <style>
        ...
        canvas { box-shadow: 0 4px 8px rgba(0,0,0,0.2); margin: 10px 0; }
    </style>
</head>
<body>
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        ...
        setDownloadUrl(url);
        // For web viewer logic if needed, but we use iframe or new tab
      } else {
        const response = await axios.post(API_URL, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        ...
        setPdfBase64(base64); // Store for Android Viewer
      ...
      console.error(error);
      let errorMessage = 'Failed to unlock PDF. Please try again.';
        ...
        <View style={styles.content}>
          ...
          {step === 'HOME' && (
            <View style={styles.card}>
              <Text style={styles.subtitle}>Select a locked PDF to unlock</Text>
              ...
              {file && (
                <View style={styles.fileInfo}>
                  ...
                  <TouchableOpacity style={[styles.button, styles.primaryButton]} onPress={handleUnlock}>
                    <Text style={styles.buttonText}>Unlock PDF 🔓</Text>
                  </TouchableOpacity>
                </View>
              )}
          ...
          {step === 'PROCESSING' && (
            <View style={styles.centerParams}>
              ...
              <Text style={styles.statusText}>Unlocking your PDF...</Text>
              <Text style={styles.hintText}>This may take up to 4-5 minutes for complex passwords.</Text>
            </View>
          )}
          ...
          {step === 'RESULT' && (
            <View style={styles.card}>
              ...
              <Text style={styles.subtitle}>Your PDF has been unlocked.</Text>

              <TouchableOpacity style={[styles.button, styles.primaryButton]} onPress={handleDownload}>
                <Text style={styles.buttonText}>Download / Save PDF</Text>
              </TouchableOpacity>

              <TouchableOpacity style={[styles.button, styles.secondaryButton, { marginTop: 12 }]} onPress={() => setShowViewer(true)}>
                <Text style={[styles.buttonText, styles.secondaryButtonText]}>Preview PDF 👁️</Text>
              </TouchableOpacity>

              <TouchableOpacity style={[styles.button, styles.secondaryButton, { borderStyle: 'dashed', borderColor: '#9CA3AF', marginTop: 12 }]} onPress={() => { setStep('HOME'); setFile(null); }}>
                <Text style={[styles.buttonText, { color: '#4B5563' }]}>Unlock Another</Text>
              </TouchableOpacity>
            </View>
          )}
          ...
          <Modal visible={showViewer} animationType="slide">
            <SafeAreaView style={{ flex: 1, backgroundColor: '#f3f4f6' }}>
              ...
              <View style={{ flex: 1 }}>
                {Platform.OS === 'web' ? (
                  <iframe src={downloadUrl} style={{ width: '100%', height: '100%', border: 'none' }} />
                ) : Platform.OS === 'android' ? (
                  <WebView
                    source={{ html: PDF_VIEWER_HTML.replace('{PDF_BASE64}', pdfBase64 || '') }}
                    style={{ flex: 1 }}
                    originWhitelist={['*']}
                  />
                ) : (
...
const styles = StyleSheet.create({
  ...
  fileInfo: { marginTop: 24, width: '100%', alignItems: 'center', padding: 16, backgroundColor: '#F9FAFB', borderRadius: 12 },
  fileName: { fontSize: 16, fontWeight: '600', color: '#374151', marginBottom: 4, textAlign: 'center' },
  statusText: { marginTop: 20, fontSize: 20, fontWeight: '600', color: '#4F46E5', textAlign: 'center' },
  hintText: { marginTop: 8, color: '#6B7280', textAlign: 'center' },
  successTitle: { fontSize: 24, fontWeight: 'bold', color: '#10B981', marginBottom: 12 },
  errorText: { color: '#DC2626', textAlign: 'center', marginTop: 10 },
  modalHeader: { padding: 16, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'white', borderBottomWidth: 1, borderBottomColor: '#E5E7EB' },
  modalTitle: { fontSize: 18, fontWeight: 'bold' },
  closeButton: { fontSize: 16, color: '#4F46E5', fontWeight: '600' }
});
